...
PixelShader =
{
	...
	[[
		...
		float4 main( VS_OUTPUT_WATER Input ) : PDX_COLOR
		{
			...
			refractiveColor = lerp(refractiveColor, Darkness, 0.35f);
			float waterLuma = dot( refractiveColor, float3( 0.299f, 0.587f, 0.114f ) );
			refractiveColor = lerp( refractiveColor, float3( waterLuma, waterLuma, waterLuma ), 0.7f );
			...
			float sceneLuma = dot( vOut, float3( 0.299f, 0.587f, 0.114f ) );
			float3 steelTint = float3( sceneLuma * 0.86f, sceneLuma * 0.9f, sceneLuma * 0.98f );
			vOut = lerp( vOut, steelTint, 0.34f );
			vOut *= 1.1f;
