...
{	
	...
	[[		
		float4 main( VS_OUTPUT_TERRAIN Input ) : PDX_COLOR
		{
			...
			normal = normalize( lerp( float3( 0, 1, 0 ), normal, 0.7f ) );
		#endif
			...
			//return float4(terrain_normal.rgb, 1.0f);
				...
				float vSpec = 0.0f;
			#else
				float vSpec = terrain_normalRaw.a * 0.7f;
				vGlossiness = diffuse.a * 0.9f;
			#endif
		#else
			float vSpec = 0.0f;
		#endif
		...
		#ifndef NO_SHADER_TEXTURE_LOD
			terrain_normal = normalize( lerp( float3( 0, 1, 0 ), terrain_normal, 0.72f ) );
			...
			float3 steelTint = float3( sceneLuma * 0.9f, sceneLuma * 0.94f, sceneLuma * 1.0f );
			vOut = lerp( vOut, steelTint, 0.2f );
			vOut *= 1.08f;
